...
public class HomeFragment extends Fragment implements OnMapReadyCallback{
   ...
   MapsFragment mapsFragment = new MapsFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        adapter2 = new MyAdapter(ct, strTitle2, strDescription2, image);
        listView2.setAdapter(adapter2);


        MyAdapter adapter = new MyAdapter(ct, strTitle, strDescription, image);
        adapter2 = new MyAdapter(ct, strTitle2, strTitle2, image);


        listView.setAdapter(adapter);


        search_et = (EditText) v.findViewById(R.id.editText);
        searchBtn = (ImageButton) v.findViewById(R.id. buttonSearch);
            ...
            private void search(String charText) {
                ...
                else
                {
                    for (int i = 0; i < strTitle.length; i++)
                        if (strTitle[i].toLowerCase().contains(charText)) {
                            int n = 0;
                            strTitle2[n] = strTitle[i];
                            strDescription2[n] = strDescription2[i];
                            n++;
                        }
                }
